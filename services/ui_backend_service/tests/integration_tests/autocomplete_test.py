...
async def test_flows_autocomplete(cli, db):
    ...
    await _test_list_resources(
        cli, db, "/flows/autocomplete", 200, ["HelloFlow", "HelloFlow2", "HelloFlow3"]
    )

    # Partial matches
    await _test_list_resources(
        cli, db, "/flows/autocomplete?flow_id:co=low2", 200, ["HelloFlow2"]
    )
...
async def test_runs_autocomplete(cli, db):
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/HelloFlow/runs/autocomplete",
        200,
        ["HelloRun", "HelloRun2", "HelloRun3"],
    )

    # Partial matches
    await _test_list_resources(
        cli, db, "/flows/HelloFlow/runs/autocomplete?run:co=Run2", 200, ["HelloRun2"]
    )

    # no-match
    await _test_list_resources(
        cli, db, "/flows/HelloFlow/runs/autocomplete?run:co=test", 200, []
    )
...
async def test_steps_autocomplete(cli, db):
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flowid}/runs/{runid}/steps/autocomplete".format(
            flowid=_step.get("flow_id"), runid=_step.get("run_number")
        ),
        200,
        ["another_step", "regular_step", "step3"],
    )
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flowid}/runs/{runid}/steps/autocomplete?step_name:co=lar".format(
            flowid=_step.get("flow_id"), runid=_step.get("run_number")
        ),
        200,
        ["regular_step"],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flowid}/runs/{runid}/steps/autocomplete?step_name:co=test".format(
            flowid=_step.get("flow_id"), runid=_step.get("run_number")
        ),
        200,
        [],
...
async def test_artifacts_autocomplete(cli, db):
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flowid}/runs/{runid}/artifacts/autocomplete".format(
            flowid=_task.get("flow_id"), runid=_task.get("run_number")
        ),
        200,
        ["first", "first3", "second"],
    )
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flowid}/runs/{runid}/artifacts/autocomplete?name:co=rst".format(
            flowid=_task.get("flow_id"), runid=_task.get("run_number")
        ),
        200,
        ["first", "first3"],
    ...
    await _test_list_resources(
        cli,
        db,
        "/flows/{flowid}/runs/{runid}/artifacts/autocomplete?name:co=test".format(
            flowid=_task.get("flow_id"), runid=_task.get("run_number")
        ),
        200,
        [],
