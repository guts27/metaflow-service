...
async def test_step_post(cli, db):
    ...
    _found = (
        await db.step_table_postgres.get_step(
            _step["flow_id"], _step["run_number"], _step["step_name"]
        )
    ).body
    ...
    await assert_api_post_response(
        cli,
        path="/flows/NonExistentFlow/runs/{run_number}/steps/test_step/step".format(
            **_run
        ),
        payload=payload,
        status=500,
...
async def test_steps_get(cli, db):
    ...
    await assert_api_get_response(
        cli,
        "/flows/{flow_id}/runs/{run_number}/steps".format(**_first_step),
        data=[_first_step, _second_step],
    ...
    await assert_api_get_response(
        cli,
        "/flows/NonExistentFlow/runs/{run_number}/steps".format(**_first_step),
        status=200,
        data=[],
    )
...
async def test_step_get(cli, db):
    ...
    await assert_api_get_response(
        cli,
        "/flows/NonExistentFlow/runs/{run_number}/steps/{step_name}".format(**_step),
        status=404,
    )
    await assert_api_get_response(
        cli, "/flows/{flow_id}/runs/1234/steps/{step_name}".format(**_step), status=404
    )
    await assert_api_get_response(
        cli,
        "/flows/{flow_id}/runs/{run_number}/steps/nonexistent_step".format(**_step),
        status=404,
    )
